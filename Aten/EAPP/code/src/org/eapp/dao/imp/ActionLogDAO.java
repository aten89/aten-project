 ...
 * @version
 */
public class ActionLogDAO extends BaseHibernateDAO implements IActionLogDAO {
    private static final Log log = LogFactory.getLog(ActionLogDAO.class);
	
	/**
	 * 日志表名
	 */
	private static final String LOG_TABLE_NAME = "EAPP_ACTION_LOG";
    ...
    public void createLogTable(int year) {
		...
		query.executeUpdate();
		//创建ACTIONLOG_ID索引
		query = getSession().createSQLQuery("create index LOG_ID_A"+ year + " on " + tableName + " (LOG_ID_ ASC)");
		query.executeUpdate();
		//创建SYSTEMID_索引
		query = getSession().createSQLQuery("create index SYSTEM_ID_A" + year + " on " + tableName + " (SYSTEM_ID_ ASC)");
		query.executeUpdate();
		//创建MODULEKEY_索引
		query = getSession().createSQLQuery("create index MODULE_KEY_A" + year + " on " + tableName + " (MODULE_KEY_ ASC)");
		query.executeUpdate();
		//创建ACTIONNAME_索引
		query = getSession().createSQLQuery("create index ACTION_KEY_A" + year + " on " + tableName + " (ACTION_KEY_ ASC)");
		query.executeUpdate();
		//创建ACTORACCOUNTID_索引
		query = getSession().createSQLQuery("create index ACCOUNT_ID_A" + year + " on " + tableName + " (ACCOUNT_ID_ ASC)");
		query.executeUpdate();
		//创建ACTORDISPLAYNAME_索引
		query = getSession().createSQLQuery("create index ACCOUNT_NAME_A" + year + " on " + tableName + " (ACCOUNT_NAME_ ASC)");
		query.executeUpdate();
		//创建OPERATETIME_索引
		query = getSession().createSQLQuery("create index OPERATE_TIME_A" + year + " on " + tableName + " (OPERATE_TIME_ ASC)");
		query.executeUpdate();
    }
	...
	@Override
	public ListPage<ActionLog> queryLog(LogQueryParameters logQP) {
		if (logQP == null)  {
			throw new IllegalArgumentException();
		}
		ListPage<ActionLog> listPage = new ListPage<ActionLog>();
		...
		StringBuffer hql = new StringBuffer("from ").append(tableName)
				.append(" log where log.OPERATE_TIME_>=:beginTime and log.OPERATE_TIME_ <=:endTime");
		if (logQP.getSystemID() != null) {
			hql.append(" and log.SYSTEM_ID_=:systemID");
		}
		if (logQP.getModuleKey() != null) {
			hql.append(" and log.MODULE_KEY_=:moduleKey");
		}
		if (logQP.getActionKey() != null) {
			hql.append(" and log.ACTION_KEY_ like :actionKey");
			logQP.toArountParameter("actionKey");
		}
		...
		if (logQP.getIsServiceLog() != null) {
			hql.append(" and log.IS_SERVICE_LOG_=:isServiceLog");
		}
...
//	        CommQuery.launchParamValues(query, logQP.getParameters());
        ...
        if (totalCount > 0 && firstResultIndex < totalCount) {
          	query = session.createSQLQuery("select log.* " + hql.toString() 
          			+ " order by log.OPERATE_TIME_ desc").addEntity(ActionLog.class);
          	query.setProperties(logQP.getParameters());
//	          	CommQuery.launchParamValues(query, logQP.getParameters());
          	...
          	List<ActionLog> result = query.list();
	...
	public ActionLog getByID(String logID) {
		...
		Query query = getSession().createSQLQuery("select log.* from " + tableName 
				+ " log where log.LOG_ID_=:logID").addEntity(ActionLog.class);
		query.setString("logID", logID);
		return (ActionLog)query.uniqueResult();
	}
	...
	private void backUpLog(Calendar time) {
		...
		Query query = session.createSQLQuery("insert into " + tableName + " select l.* from " + LOG_TABLE_NAME + " l " +
				"where l.OPERATE_TIME_>=:beginTime and l.OPERATE_TIME_<=:endTime and l.IS_BACKUP_=0");
		...
		query = session.createSQLQuery("update " + LOG_TABLE_NAME + " set IS_BACKUP_=1 where " +
				"OPERATE_TIME_>=:beginTime and OPERATE_TIME_<=:endTime and IS_BACKUP_=0");
		query.setTimestamp("beginTime", beginTime);
		query.setTimestamp("endTime", endTime);
		query.executeUpdate();
	}
	...
	public void delLastYearLogs() {
		...
		Query query = getSession().createQuery("delete from ActionLog as l where l.operateTime<:time and l.isBackUp=1");
	...
	public void startUpBackUp() {
		//获取日志表中未备份的最晚日志时间			
		Query query = getSession().createQuery("select max(l.operateTime) from ActionLog as l where l.isBackUp=0 ");
