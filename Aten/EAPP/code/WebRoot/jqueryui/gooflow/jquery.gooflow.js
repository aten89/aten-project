...
//初始化区域图的对象
	...
	if(property.haveHead){
		tmp="<div class='GooFlow_head'><div>流程名称：</div><label title='"+(property.initLabelText||"newFlow_1")+"'>"+(property.initLabelText||"newFlow_1")+"</label>";
		...
		this.$head=$(tmp);
		//流程名称点击事件
		this.$head.children("label").on("click",{inthis:this},function(e){
			var This = e.data.inthis;
			This.blurItem();
			if(This.onClickTitle!=null) {
				This.onClickTitle(This.$title);
			}
		});
		...
		this.$head.on("click",{inthis:this},function(e){
			...
			switch($(tar).attr("class")){
				...
				case "ico_del":	This.delFocusNode();break;
				case "ico_expand":	This.expandWorkArea();break;
				case "ico_shrink":	This.shrinkWorkArea();break;
	...
	//以下两个函数非编辑时也可用
	//扩展工作区域
	this.expandWorkArea=function(){
		var WorkArea = this.$workArea;
		//每次扩展100
		WorkArea.width(WorkArea.width()+100);
		WorkArea.height(WorkArea.height()+100);
	};
	//缩小工作区域
	this.shrinkWorkArea=function(isCheck){
		var shrinkVal = isCheck ? 0 : 100;
		var WorkArea = this.$workArea;
		var showHeight = this.$bgDiv.height()-29;
		var showWidth = this.$bgDiv.width()-39;
		if(!property.haveTool){
			//左边工具栏不显示时最小宽度要加上被隐藏工具的宽度
			showWidth +=31;
		}
		//每次缩小100
		var toHeight = WorkArea.height()-shrinkVal;
		var toWidth = WorkArea.width()-shrinkVal;
		WorkArea.width(toWidth < showWidth ? showWidth : toWidth);
		WorkArea.height(toHeight < showHeight ? showHeight : toHeight);
	};
}
GooFlow.prototype={
	useSVG:"",
	...
	initDraw:function(id,width,height){
		...
		$(this.$draw).delegate(tmpClk,"dblclick",{inthis:this},function(e){
			...
			x=(parseInt(from[0],10)+parseInt(to[0],10))/2-50;
	...
	initGroup:function(width,height){
	  ...
	  this.$group.on("dblclick",{inthis:this},function(e){
		...
		var x=parseInt(p.style.left,10)+20,y=parseInt(p.style.top,10)+1;
		var t=getElCoordinate(This.$workArea[0]);
		This.$textArea.val(oldTxt).css({display:"block",width:100,height:14,
			left:t.left+x-This.$workArea[0].parentNode.scrollLeft,
	...
	setNodeRemarks:function(remark){
		...
		//操作栏的提示
		if (this.$head) {
			this.$head.find("b").each(function(){
					this.title=remark[$(this).attr("class").split("ico_")[1]];
			});
		}
	...
	addNode:function(id,json){
		...
		if(json.type.indexOf(" round") > 0){
			itemShape = "round";//圆形
		} else if(json.type.indexOf(" rhombus") > 0){
			itemShape = "rhombus";//菱形
		} else if(json.type.indexOf(" square") > 0){
			itemShape = "square";//正方形
		}
		if (itemShape) {
			json.width=30;json.height=30;
			this.$nodeDom[id]=$("<div class='GooFlow_item item_" + itemShape + mark + "' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='0'><tr><td class='ico'><b class='ico_"+json.type+"'></b></td></tr></table><div class='rs_close_" + itemShape +"' style='display:none'><div class='rs_close'></div></div><div class='span'>"+json.name+"</div></div>");
		} else {
			if(!json.width||json.width<86)json.width=100;
			if(!json.height||json.height<24)json.height=40;
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".GooFlow_item","mouseup",{inthis:this},function(e){
			...
			if(lineStart)	This.addLine(This.$id+"_line_"+This.$max,{from:lineStart.id,to:this.id,name:"line_"+e.data.inthis.$max});
			This.$max++;
		});
		//绑定双击编辑事件
		this.$workArea.delegate(".GooFlow_item > .span","dblclick",{inthis:this},function(e){
			...
			This.$textArea.val(oldTxt).css({display:"block",height:$(this).height(),width:100,
				left:t.left+This.$nodeData[id].left-This.$workArea[0].parentNode.scrollLeft-35,
				top:t.top+This.$nodeData[id].top-This.$workArea[0].parentNode.scrollTop+36})
		...
		this.$workArea.delegate(".ico + td","dblclick",{inthis:this},function(e){
			...
			var t=getElCoordinate(This.$workArea[0]);
			This.$textArea.val(oldTxt).css({display:"block",width:$(this).width()-2,height:$(this).height(),
				left:t.left+23+This.$nodeData[id].left-This.$workArea[0].parentNode.scrollLeft,
	...
	setName:function(id,name,type){
		...
		if(type=="node"){//如果是结点
			...
			if(this.$nodeData[id].type.indexOf("round")>1 || this.$nodeData[id].type.indexOf("square")>1 || this.$nodeData[id].type.indexOf("rhombus")>1){
	...
	loadData:function(data){
		...
		this.$deletedItem={};
		//默认工作区域
		this.$workArea.css({height:data.height+"px",width:data.width+"px"});
		this.shrinkWorkArea(true);//给定高宽小于显示区域时填满
	},
	//用AJAX方式，远程读取一组数据
	...
	exportData:function(){
		var ret={title:this.$title,nodes:this.$nodeData,lines:this.$lineData,areas:this.$areaData,initNum:this.$max,width:this.$workArea.width(),height:this.$workArea.height()};
...
///////////以下为有关画线的方法
	...
	delArea:function(id){
		...
		if(this.onItemDel!=null&&!this.onItemDel(id,"area"))	return;
	...
	addArea:function(id,json){
		if (this.$group == null) {//haveGroup=false时this.$group为空
			return;
		}
