...
//            },
        ...
        this.showBox=function (){
            ...
            B = $("<div id='wBox_overlay' class='wBox_hide'></div>").hide();
            if (_this.YQ.overlayBG) {//增加背景覆盖控制 add by zsy
            	B.addClass('wBox_overlayBG').css('opacity', _this.YQ.opacity).dblclick(function(){
	                _this.close();
	            });
            }
           	B.appendTo('body').fadeIn(300);
        ...
        function calPosition(w){
        	if(_this.YQ.position) {
        		//增加弹出位置设置 add by zsy
        		return [_this.YQ.position.left, _this.YQ.position.top];
        	} else {
	            l = ($win.width() - w) / 2;
         ...
         */
        this.close=function (){
        	var colseFlag = true;//是否关闭
        	if (typeof _this.YQ.closeCallBack === 'function') {
				colseFlag = _this.YQ.closeCallBack();
				if (typeof colseFlag == "undefined") {//没返回值，默认true
					colseFlag = true;
				}
			}
