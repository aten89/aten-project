        		...
        		if(data.portlets){
					...
					if (dataList) {
						$(dataList).each(function(i) {
							var portletID = dataList[i].portletID;
							...
							$(".portMain","#_customPortletDIV").append("<div id='" + portletID + "' width='" + width + "' height='" + height + "' class='portOp' style='width:" + width + ";height:" + height + ";'><div class='portDiv'><h1>"+dataList[i].portletName+"</h1><input type='hidden' value='" + portletID + "'/><div></div>");
	        			});
	        		}
        		}
        	}
			...
			if ($.checkErrorMsg(data) ) {
        		if(data.portlets){
					...
					if (dataList) {
						$(dataList).each(function(i) {
							var portletID = dataList[i].portletID;
							if ($("#" + portletID, "#_customPortletDIV").length == 0) {
								var style = dataList[i].style;
		                		var width = "33.3%";
		                		var height = "104px";
		                		if (!style){
								...
								$("#_noCustomPortletDIV").append("<div id='" + portletID + "' width='" + width + "' height='" + height + "' class='portOp' style='width:110px;height:18px;'><div class='portDiv'><h1>"+dataList[i].portletName+"</h1><input type='hidden' value='" + portletID + "'/></div></div>");
							 }
						});
					}
        		}
