...
function initBindUser(){
    ...
    $("#addSubPost,#sortSub,#editPost,#delPost,#bindRole").removeAttr("disabled","true").removeClass("icoNone");
    ...
    $("#saveBind").click(
    	function(){
    		$.ajax({
                ...
                async : true,
                dataType : "json",
        		url  : "m/post/binduser" + (postId?"?postID=" + postId : ""),
...
//			var groupId = $.trim($("#hidGroupId").val());
   	...
   	//回车搜索
   	$.EnterKeyPress($("#userKeyword"),$("#searchPostUsers")); 
   	
	//取得已绑定的用户
    getBindedUsers(postId);        
}

//取得绑定用户
function getBindedUsers(postId){
    $.ajax({
        type : "POST",
        cache: false,
        url  : "m/post/bindedusers",
        dataType : "json",
