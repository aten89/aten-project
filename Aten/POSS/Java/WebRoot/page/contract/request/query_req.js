	...
	$.ajax({
	        ...
	        success : function(data,i){
	        	...
	        	if ($.checkErrorMsg(data)) {
	        		if(data.contractRequestStatPage && data.contractRequestStatPage.dataList){
					var dataList = data.contractRequestStatPage.dataList;
			    	$(dataList).each(function(i) {
		           			...
		           			"<td class=\"listData\">"+dataList[i].signRateStr+"</td>";
		            ...
		            $("#data_list").html(html);
		            trunPageObj.setPageData(data.contractRequestStatPage);
	        	}
	        }
	    });
}
...
function addContractDetail(){
	...
	var title = "登记所需合同";
	var url = BASE_PATH + "/m/cont_request/initedit";
	var width = 560;
	var height = 260;
...
function viewRegDetail(prodId, prodName, orgName){
	...
	var url = BASE_PATH + "/m/cont_request/initquerydetail?prodId="+prodId;
	var width = 1000;
	var height = 400;
	...
	$.confirm("是否确认删除?",function(result) {
		if (result) {
			$.ajax({
				...
				cache: false,
				async: true,
				url : BASE_PATH + "/m/cont_request/delete_all",
				dataType : "json",
				data:{
					prodId : prodId
				},
