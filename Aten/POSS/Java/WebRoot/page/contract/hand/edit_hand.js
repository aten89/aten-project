...
function saveDetail(){
	...
	if ($.validInput("handDate", "上交时间", true)) {
		return false;
	}
	if ($.validInput("handRemark", "上交备注", false,"",1000)) {
		return false;
	}
	...
	$.ajax({
		   type : "POST",
		   cache: false, 
		   url  : BASE_PATH + "/m/cont_hand/"+ (id ? "modify":"add"), 
		   dataType : "json",
		   data : {
				prodId : prodId,
				id : id,
				...
				handDate : $("#handDate").val(),
				handRemark : $("#handRemark").val()
		   },
