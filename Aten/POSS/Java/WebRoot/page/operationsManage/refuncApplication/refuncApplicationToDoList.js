...
function queryList(){
	...
	$.ajax({
	        type : "POST",
			cache: false,
			url  : "m/operationsManage/queryCustRefundDealList",
			dataType : "json",
			data:{
				pageNo : pageno,
				pageSize : pagecount,
			        ...
			        if(page != null && page.dataList!=null){
			           ...
			           for(var i =0; i<list.length; i++){
			        	   ...
			        	   var taskid = list[i].task.id;
			        		...
			        		"<td class=\"listData\">" +
			        		($.hasRight($.OaConstants.DISPOSE)? "<a class=\"opLink\" href=\"javascript:void(0);\" onclick=\"initCustRefund('" + taskid + "', '"+tiid+"')\">办理</a> " : "") + 
			        		"</td></tr>";
			           }
			        }
		            $("#refuncApplicationToDoList").html(html);
...
//处理页面
function initCustRefund(taskid, tiid) {
	mainFrame.addTab({
		id:"dealCustRefund" + taskid,
		title:"处理退款申请",
		url:BASE_PATH + "m/operationsManage/viewCustRefund?taskid=" + taskid + "&tiid=" + tiid,
		callback:queryList
	});
}