   ...
   $("#openUserSelect").click(
	   	function(e){
			...
			dialog.setCallbackFun(function(user){
				if (user != null) {
					$("#saleManId").val(user.id);
					$("#saleManName").val(user.name);
				}
			});
  	...
  	formStatusSel = $("#formStatusDiv").ySelect({width:60});
...
//	var ascend =  columnSortObj.getAscend();
	...
	$.ajax({
	        type : "POST",
			cache: false,
			url  : "m/operationsManage/queryAllCustPaymentList",
			dataType : "json",
			data:{
				flag : standardFlag,
...
//				custId : customerId,
				...
				formStatus : formStatusSel.getValue(),
				prodId : prodId,
				payType : payType,
				pageNo : pageno,
				pageSize : pagecount,
				sortCol : "transferDate",
				ascend : "false"
			},
			           ...
			           for(var i =0; i<list.length; i++){
			        	   var id = list[i].id;
			        		...
			        		"<td class=\"listData\">" + (list[i].cusManageName==null ? "" : list[i].cusManageName) + "</td>" +
		           			...
		           			"<td class=\"listData\">" + getFormStatus(list[i].formStatus) + "</td>" +
			        		"<td class=\"listData\">" +
