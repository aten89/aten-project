...
//	columnSortObj = $("#tableTH").columnSort();
	...
	trunPageObj.gotoPage(1);
  	...
  	// 放最后初始化
	// 初始化状态下拉框
	statusSel = $("#statusSel").ySelect({width : 60});
...
function initView(id) {
	var dataSource = $("#dataSource").val();
	var moduleKey = dataSource == "company_allot" ?"COMPANY_ALLOT":"MY_DRAFT_CUST";
	mainFrame.addTab({
		id:"viewCustomerInfo"+id,
		title:"客户资料",
		url:BASE_PATH + "page/MyCustomer/CustomerInfo/customerInfoFrame.jsp?customerId=" + id +"&moduleKey=" +moduleKey,
		callback:queryList
	});
}
