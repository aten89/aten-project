...
public class CommonFileArchCtrl extends HttpServlet {
    ...
    private void queryArchDocForm(HttpServletRequest request, HttpServletResponse response) throws ServletException,
            IOException {
        ...
        String fileType = HttpRequestHelper.getParameter(request, "fileType");
        String draftsmanName = HttpRequestHelper.getParameter(request, "draftsmanName");
        String group = HttpRequestHelper.getParameter(request, "group");
        ...
        qp.setDocClassName(fileType);
        qp.setDraftsman(draftsmanName);
        qp.setGroupName(group);

        qp.addOrder("archiveDate", false);
        ListPage<DocForm> list = docFormBiz.getArchDocForm(qp, user.getAccountID(), DocForm.COM_FILE);
        XMLResponse.outputXML(response, new DocFormPage(list).toDocument());
    }

    public void viewDoc(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (tasks != null && tasks.size() > 0) {
                Task t = tasks.get(0);
                request.setAttribute("flowName", t.getFlowName());
            }
            request.setAttribute("tasks", tasks);
            request.setAttribute("docForm", docForm);

            request.getRequestDispatcher("/page/doc/common/view_doc.jsp").forward(request,
                    response);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void initModifyInfo(HttpServletRequest request, HttpServletResponse response) throws ServletException,
            IOException {
    	SessionAccount user = (SessionAccount) request.getSession().getAttribute(SysConstants.SESSION_USER_KEY);
        if (user == null) {
            XMLResponse.outputStandardResponse(response, 0, "请先登录");
