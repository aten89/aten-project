 ...
 * @version
 */
public class PositiveDealCtrl extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (SysConstants.QUERY.equalsIgnoreCase(action)) {
			//查询待处理的请假单
			queryDealingPositiveApply(request, response);
			return;
			...
			//任务处理页面——审批
			initPositiveApprove(request, response);
			...
			//申请人处理修改报销单任务
			dealModifyPositiveApply(request, response);
			return;
		} 
	...
	private void queryDealingPositiveApply(HttpServletRequest request,
		...
		List<PositiveApply> holidayApply = positiveApplyBiz.getDealingPositiveApply(user.getAccountID(), userRoles);
		XMLResponse.outputXML(response, new PositiveApplyList(holidayApply).toDocument());
	}
	...
	private void initPositiveApprove(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		SessionAccount user = (SessionAccount)request.getSession().getAttribute(SysConstants.SESSION_USER_KEY);
		if (user == null) {
			XMLResponse.outputStandardResponse(response, 0, "请先登录");
		...
		try {
			...
			if ("m".equals(flag)) {//发起人导修改
				pageUrl = "/page/hr/positive/tview_modify.jsp";
			} else {
				pageUrl = "/page/hr/positive/tview_dispose.jsp";
			}
	...
	private void dealModifyPositiveApply(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			positiveApplyBiz.txDealApproveTask(taskInstanceId, comment, transitionName, reDraft);
