 ...
 * @version Nov 30, 2008
 */
public class StaffDeptManagerDecision implements IDecisionHandler {
	public static final String TRANSITION_YES = "是";
	public static final String TRANSITION_NO = "否";
	
	private static final Log log = LogFactory.getLog(StaffDeptManagerDecision.class);
	
	@Override
	public String decide(Execution exe) throws Exception {
		log.info("FGManagerDecision:" + exe.getNode().getName());
		
		ContextVariable var = exe.getFlowInstance().getVariable(SysConstants.FLOW_VARNAME_STAFFUSERACCOUNTID);
		String userAccountId = var.getValue();//离职人
		if (userAccountId == null) {
			log.warn("上下文变量为空：" + SysConstants.FLOW_VARNAME_STAFFUSERACCOUNTID);
