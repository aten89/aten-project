    ...
    $("#saveUserRoles").click(
        function(e){
            ...
            $.ajax({
                ...
                cache: false,
                async : true,
        		url  : "m/info_param?act=bindpost&id=" + $("#id").val()+"&flag="+flagArgs,
                data : posts,
...
function loadBindedPost(){
	$.ajax({
		...
		async : true,
		url  : "m/info_param",
		data : {act:"get_binding_post",id:$("#id").val(),flag:flagArgs},
		success : function(xml){
		    var message = $("message",xml);
		    if (message.attr("code") == "1") {
		          var tbodyHtml = "";
...
function initPostTree(){
	...
	/*
    $("#postTree > li > ul").load(ERMP_PATH + "/m/post?act=subposts" + "&path=" + ERMP_PATH,
        //AJAX加载完毕时，进行树的初始化
        function(){
            $("#postTree").simpleTree({
                animate: true,
