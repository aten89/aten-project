...
function commitDispose(){
	...
	if(amendFlag){
		$("div.feesSp").each(function(){
			...
			var discardType=$.trim($("select[name='discardTypeSel']", this).val());
			var remaining=$.trim($("input[name='remaining']", this).val());
			var discardDate=$.trim($("input[name='discardDate']", this).val());
			...
			if(discardType==""){
				$.alert(deviceName+"报废类型不能为空")
				flag=true;
				return false;
			}
			if(validatePriceField(remaining,deviceName+"余值",true)){
					flag=true;
					return false;
			}
			...
			if(discardDate==""){
				$.alert(deviceName+"报废日期不能为空")
				flag=true;
				return false;
			}
			devDetail.deviceID = $("span[name='deviceID']", this).text();
			devDetail.remaining = remaining;
			devDetail.discardType = discardType;
			devDetail.discardDate = discardDate;
