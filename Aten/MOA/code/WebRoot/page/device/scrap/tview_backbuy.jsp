...
<script type="text/javascript" src="page/device/scrap/tview_backbuy.js"></script>
...
<div class="tabMid"> 
  ...
  <c:if test="${list.dealType=='SCRAP_DISPOSE_TAKE' || list.dealType=='SCRAP_DISPOSE_LOSE' || list.dealType=='SCRAP_DISPOSE_TAKEAWAY'}">
  <div class="feesSp">
    <table cellspacing="0" cellpadding="0" border="0" width="100%" >
      <tr>
        <td class="spNum"><span name="orderNum">${devList.displayOrder }</span></td>
        <td colspan="6" class="spOpW opw2">&nbsp;<span name="dealType" style="display: none">${list.dealType}</span></td>
      </tr>
      <tr>
        <td width="70" class="spTit">设备编号：</td>
        <td width="128">${list.device.deviceNO }</td>
        <td width="70" height="27" class="spTit">设备名称：</td>
        <td width="100"><span name="deviceName">${list.device.deviceName }</span><span name="deviceID" style="display: none">${list.device.id }</span><span name="listID" style="display: none">${list.id }</span></td>
        <td width="70" class="spTit">购买方式：</td>
        <td width="390">${list.device.buyTypeName }</td>
        <td rowspan="2">&nbsp;</td>
      </tr>
      <tr>
                    ...
                    <div class="shadowCon shadowPadd">
                      <table width="100%" border="0" cellspacing="1" cellpadding="0">
                        <tr>
                          ...
                          <th width="120"  class="sbRight">余值</th>
                        ...
                        <tr>
                          <th width="80" class="sbRight"><span class="cRed">*</span>回购价格</th>
                          <td width="130"><input name="buyPrice" type="text" maxlength="12" class="ipt05 iptSo" value="<fmt:formatNumber value="${list.buyPrice }" pattern="0.00"/>"/>元</td>
                           <th width="120" class="sbRight"><span class="cRed">*</span>预期付款日期</th>
                          <td width="130" colspan="5"><input type="text" name="planPayDate" maxlength="100" readonly="readonly" class="invokeBoth"  style="width:65px" value="<fmt:formatDate value="${list.planPayDate }" pattern="yyyy-MM-dd"/>"/></td>
                        </tr>
                        </c:if>
                         <c:if test="${list.dealType=='SCRAP_DISPOSE_LOSE'}">
                        <tr>
                           <th width="120" class="sbRight"><span class="cRed">*</span>预期付款日期</th>
                          <td width="130" colspan="7"><input type="text" name="planPayDate" maxlength="100" readonly="readonly" class="invokeBoth"  style="width:65px" value="<fmt:formatDate value="${list.planPayDate }" pattern="yyyy-MM-dd"/>"/></td>
                        </tr>
                        </c:if>
                      </table>
                    </div>
