...
function initEquipmentInfoAllotEdit() {
	...
	if ($("#formType").val() == "1") {
		document.title = "离职处理单";
		$(".tipBg").html("<img src=\"themes/comm/images/frameNav2.gif\"/>离职处理单");
	}
...
function displayOrhide(vv){
	...
	if(vv == "radDevPurchase" && $("#" + vv)[0].checked){
		...
		$("#areaDeviceTR").show();
		initBuyType();
		initDeviceClassSel($.trim($("select[name='deviceTypeSel']").val()));
	}
}	
...
function selectDevice() {
	var deviceTypeCode = deviceTypeSel == null ? "" : $("select[name='deviceTypeSel']").val();
...
//	}
	...
	args.excludeSelfBuyFlag = false;//不排除个人全额购买的设备
	...
	dialogWin = $.showModalDialog("设备选择", sURL, 805, 378, function(){
...
function saveInfo(startFlow){
	...
	var deviceTypeCode = $("select[name='deviceTypeSel']").val();
	/*
...
function validateForm() {
	...
	$("div.feesSp").each(function(){
		...
		if($("#formType").val()=="0"){
			var reason = $.trim($("input[name='reason']", this).eq(0).val());
			if(reason.length==0){
				$.alert("编号为“" + $("span[name='deviceNO']", this).eq(0).text() +  "”设备的“报废原因”不能为空")
				passedList = false;
				return false;
			}
			var result = $.validChar(reason,"\<\>\'\"");
			if (result){
...
function createDealTypeOption(url,data, defaultName, selectKey, isCompBuy) {
	...
	var deviceTypeCode = deviceTypeSel == null ? $.trim($("#deviceTypeCode").val()) : $("select[name='deviceTypeSel']").val();
...
function createLeaveDealTypeOption(url,data, defaultName, selectKey) {
	if (!defaultName) {
		defaultName = "请选择...";
	}
	var deviceTypeCode = deviceTypeSel == null ? "" : $("select[name='deviceTypeSel']").val();
...
function createScrapDeviceDetailHTML(scrapDevices, formType) {
	...
	$.each(scrapDevices, function(i, scrapDevice) {
		...
		sHtml += "</tr>";
	    sHtml += "  <tr>";
	    sHtml += "    <td width=\"70\" height=\"27\" class=\"spTit\">设备编号：</td>";
	    sHtml += "    <td width=\"128\"><span name='deviceNO'>" + scrapDevice.deviceNO + "</span></td>";
	    sHtml += "    <td width=\"70\"  class=\"spTit\">设备名称：</td>";
	    sHtml += "    <td width=\"100\"><span name='deviceName'>" + scrapDevice.deviceName + "</span><span name='deviceID' style=\"display:none;\">" + scrapDevice.deviceID + "</span></td>";
	    ...
	    if (formType == "0") {
	    	...
	    	dealTypeOptionsHTML = createLeaveDealTypeOption("m/data_dict?act=leavedealtypesel", "", "", "", "");
	    }
	    ...
	    sHtml += "    <td class=\"spTit\">" + (formType == "0" ? "<span class=\"cRed\">*</span>报废" : "") + "原因：</td>";
