...
var deviceTypeSel;
$(initTrackEquipmentInfoLeaveDeal);

function initTrackEquipmentInfoLeaveDeal() {
  //初始化表头列排序
		...
		if (message.attr("code") == "1") {
			...
			$("device-flow", xml).each(function (index) {
				...
				tBodyHTML += createTR($.trim(deviceFlow.attr("id")), 
							 ...
							 $.trim($("transactor", deviceFlow).text()),
							 $.trim($("create-time", deviceFlow).text())) 
			});
 ...
 * HTML 构造器
 */
function createTR(formID,formCode, nodeName, formTypeName,deviceTypeName,userID, groupName ,aplyDate,formType,transactor,createTime) {
...
function openDetail(formID,formType,formCode){
	...
	mainFrame.addTab({
		...
		url:BASE_PATH +"m/dev_track?act=view&formId=" + formID+"&formType="+formType,
		callback:queryList
	});
}
