...
function initEquipmentInfoScrap() {
	...
	initScrapTypeSel();
	initScrapDisposeTypeSel(deviceTypeCode,"","");
 ...
 */
function initScrapTypeSel(scrapType) {
	scrapTypeSel = $("#scrapTypeSel").ySelect({
		width : 107, 
		url:BASE_PATH+"m/data_dict?act=selectscraptype",
		afterLoad : function() {
...
function initScrapDisposeTypeSel(deviceType,scrapDisposeType, buyType) {
	scrapDisposeTypeSel = $("#scrapDisposeTypeSel").ySelect({
		width : 107, 
		url:BASE_PATH+"m/data_dict?act=leavedealtypesel&deviceType="+deviceType,
		afterLoad : function() {
			...
			if (buyType == null || buyType == "") {
				...
				//公司全额
        		$("#scrapDisposeTypeSel input[value='LEAVE_DISPOSE_TAKE']").parent().hide();
        		$("#scrapDisposeTypeSel input[value='LEAVE_DISPOSE_TOSTORAGE']").parent().hide();
			} else if (buyType == "BUY-TYPE-SELF") {
				//公司全额
        		$("#scrapDisposeTypeSel input[value='LEAVE_DISPOSE_BACKBUY']").parent().hide();
        		$("#scrapDisposeTypeSel input[value='LEAVE_DISPOSE_TOSTORAGE']").parent().hide();
			} else if (buyType == "BUY-TYPE-SUB") {
				//公司全额
        		$("#scrapDisposeTypeSel input[value='LEAVE_DISPOSE_TAKE']").parent().hide();
			}
		},
		onChange : function(value,text) {
			if (value == "LEAVE_DISPOSE_BACKBUY") {
				$("#trBackBuy").show();
			} else {
				$("#trBackBuy").hide();
			}
...
function saveInfo(){
	...
	if(validatePriceField(depreciation,deviceName+"折旧价格",true)){
		return false;
	}
	...
	var enterCompanyDate=$.trim($("#enterCompanyDate").val());
   	...
   	if (scrapDisposeType == "LEAVE_DISPOSE_BACKBUY") {
   		if(enterCompanyDate==""){
			$.alert("入司时间不能为空")
	  		return false;
	  	}
