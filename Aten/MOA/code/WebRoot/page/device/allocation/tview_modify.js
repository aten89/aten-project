...
$(initEquipmentInfoAllotAmend);

function initEquipmentInfoAllotAmend() {
...
//	initAllotTypeSel($.trim($("#moveType").val()));
	initAllotTypeSel(deviceTypeCode,$.trim($("#moveType").val()));
...
function initDeviceClassSel(deviceTypeCode, deviceClassCode) {
	deviceClassSel = $("#deviceClassSel").ySelect({
		width : 107, 
		url:BASE_PATH+"m/device_class?act=classselect&deviceTypeCode=" + deviceTypeCode,
		afterLoad : function() {
...
function saveInfo(validFormFlag){
	var bSuccess = false;
	if (validFormFlag && !validateForm()) {
		return;
	}
...
function delItem(o) {
	$.confirm("是否确认删除？", function(r){
		if (r) {
			...
			//追加deleteIDs
			var deleteDeviceID = $("input[name='deviceID']", oDel.get(0)).val();
...
function createAllotDeviceDetailHTML(allotDevices) {
	...
	$.each(allotDevices, function(i, allotDevice) {
		...
		sHtml += "</tr>";
	    sHtml += "  <tr>";
	    sHtml += "    <td height=\"27\" class=\"spTit\" style=\"width:auto\">设备名称：</td>";
	    sHtml += "    <td width=\"128\"><span name=\"deviceName\">" + allotDevice.deviceName + "</span><input type=\"hidden\" name=\"deviceID\" value=\"" + allotDevice.deviceID + "\"/>";
	    ...
	    sHtml += "    <td class=\"spTit\" style=\"width:auto\">设备编号：</td>";
	    sHtml += "    <td width=\"150\">" + allotDevice.deviceNO + "</td>";
	    sHtml += "    <td width=\"70\">设备型号：</td>";
	    ...
	    if(allotDevice.devCfgDesc!=null && allotDevice.devCfgDesc!=""){
		    sHtml += "  <tr>";
		    sHtml += "    <td class=\"spTit\" style=\"width:auto\">配置信息：</td>";
		    sHtml += "    <td colspan=\"5\" width=\"360\"><span name=\"devCfgDesc\">" + allotDevice.devCfgDesc + "</span></td>";
		    sHtml += "  </tr>";
	    }
	    sHtml += "  <tr>";
	    sHtml += "    <td class=\"spTit\" height=\"27\">设备类别：</td>";
	    sHtml += "    <td><span name=\"listDeviceClassName\">" + allotDevice.deviceClassName + "</span><input type=\"hidden\" name=\"listDeviceClassID\" value=\"" + allotDevice.deviceClassID + "\"/></td>";
//	    sHtml += "    <td class=\"spTit\" style=\"width:120px\">调拨前设备用途：</td>";
//	    sHtml += "    <td >" + allotDevice.purposeNameBef + "</td>";
	    sHtml += "    <td  class=\"spTit\" style=\"width:120px\">调拨前工作所在地：</td>";
	    sHtml += "    <td colspan=\"3\">" + allotDevice.areaNameBef + "</td>";
	    sHtml += "  </tr>";
	    sHtml += "</table>";
