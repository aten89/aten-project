...
function initControl(){
	...
	NTKO_OFFICE_OCX.setSaveURL(BASE_PATH + "m/doc_class?act=uploadtemplate&referid="+docClassId);
...
//	NTKO_OFFICE_OCX.setOperator($("#userId").val());
	...
	//NTKO_OFFICE_OCX.openOfficeFile(BASE_PATH + $("#bodyTemplateUrl").val());
	if("" == $("#bodyTemplateUrl").val()) {
		NTKO_OFFICE_OCX.openOfficeFile(BASE_PATH + "page/template/emptyDocTemplate.doc","draft");
	}
	else {
		NTKO_OFFICE_OCX.openOfficeFile(BASE_PATH + $("#bodyTemplateUrl").val(),"draft");
	}
...
function saveOfficContent(){
	var obj = NTKO_OFFICE_OCX.saveOffice(docClassId);
}
