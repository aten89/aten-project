...
function initDocLayouts(){
	...
	$.ajax({
	    ...
	    cache: false,
	    async : true,
	    url  : "m/doc_class?act=getassigndoc&fileClass=1",
	            ...
	            if($("#officeDocLayouts li").size() == 0) {
				    ...
				    $("#officeDocLayouts li").hover(function(){
						...
						mainFrame.addTab({
							...
							title:"新增日常文件",
							url:BASE_PATH + "/m/nor_start?act=initadd&doc=" + encodeURI($(this).text()),
							callback:queryList
						});
					});					
				}
...
function modifyDraft(id){
	mainFrame.addTab({
		...
		title:"修改日常文件",
		url:BASE_PATH + "/m/nor_start?act=initmodify&id=" + id,
		callback:queryList
	});
}
