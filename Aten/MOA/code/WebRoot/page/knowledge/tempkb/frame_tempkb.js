...
function initTempKnowledge(){
	...
	$("#addKnowledge").click(function(){
		mainFrame.addTab({
			...
			title:"新增知识信息",
			url:BASE_PATH + "/m/knowledge?act=initaddknowledge&id=" + openNodeId+"&isFinal=false",
	...
	$("#changeFinal").click(function(){
		openTreeSelDialog("changetofinal");
	});
	
	//知识点拷贝
	$("#copyKnowledge").click(function(){
		openTreeSelDialog("copyKnowledge");
	});
	
	$("#changeType").click(function(){
		openTreeSelDialog("changetype");
	});
...
function openTreeSelDialog(act){	
	var ids = "";
	...
	var selector = new KnowledgeDialog(BASE_PATH,BASE_PATH);
	selector.setCallbackFun(function(val){
		knowledgeList();
	});
	selector.openDialog(ids, act);
}
