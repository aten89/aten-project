...
//本部门请假情况
function vacationSituation(){
	if ( $("#selDiv01")[0].style.display=="none") {
	     $("#selDiv01").show();
	  }else{
	  	$("#selDiv01").hide();
	}
...
function toAddFlowNotify(user) {
	if ($("#cancelFlag").val() =="true") {
		addFlowNotify(user,'审批的销假申请', 'XJLC', '/m/holi_arch?act=toview&id=');
	} else {
		addFlowNotify(user,'审批的请假申请', 'QJLC', '/m/holi_arch?act=toview&id=');
	}
}
