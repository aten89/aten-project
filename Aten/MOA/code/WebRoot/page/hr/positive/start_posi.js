...
function initVacationApproval() {
	...
	//刷新
  	...
  	$("#toAdd").click(function(){
  		mainFrame.addTab({
			id:"oa_positive_draft",
			title:"填写转正单",
			url:BASE_PATH + "m/posi_start?act=initadd",
			callback:queryList
		});
  	});
...
function queryList() {
	$.ajax({
	    ...
	    cache: false,
	    async : true,
	    url  : "m/posi_start?act=query",
	    success : function(xml){
	        var listDate = "";
	        var message = $("message",xml);
	        if(message.attr("code") == "1"){
	          $(xml).find('PositiveApply').each(function(index){
		    		var curELe = $(this);
...
function modifyDraft(id){
	mainFrame.addTab({
		id:"oa_positive_modify"+id,
		title:"修改转正单",
		url:BASE_PATH + "m/posi_start?act=initmodify&id=" + id,
		callback:queryList
	});
};
