...
function queryPage() {
	...
	$.ajax({
	   ...
	   async : true,
	   url  : "m/staff_prompt",
	   data : "act=query_contr&pageno="+pageno+"&pagecount="+pagecount+"&userDeptName="+userDeptName+"&userAccountId="+userAccountId,             
		    ...
		    if (message.attr("code") == "1") {
		    	...
		    	$(xml).find('staff-flow').each(function(index){
                    ...
                    listDate += " <tr onmouseout=\"$(this).removeClass('over');\" onmouseover=\"$(this).addClass('over');\">"
							...
							+ "<td>" + $("contractEndDate",curELe).text() + "</td>"
							+ "<td>" + $("statDays",curELe).text() + "</td>"
							+ "<td>" + $("contractType",curELe).text() + "</td>"
