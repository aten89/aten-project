...
*	   3）delete:允许删除附件；
* 用法：
*	1）在需要集成附件控件的页面中，引用.oa.attachment.web.js（如下）：
*		<script language="javascript" src=".oa.attachment.json.js" />
...
function NTKOAttachmentControl(){
	...
	*/
	this.setFileList = function(data){
		...
		$(data).each(
			function(index){
				var fileName = data[index].displayName + data[index].fileExt;
				var fileSize =  data[index].size;
				var fileURL = basePath +  "/" + data[index].absFilePath;
				var modTime =  data[index].uploadDate;
				fileCount += 1;
	...
	*/
	...
	this.afterSaveAttachment = function(message, code, ctrlId){
		...
		if (code == 0){
			...
			if (data.msg.code == 1){
				//成功，触发成功事件
				eval(obj.afterSaveEvent + "(1,'" + data.msg.text + "')");
			}else{
				//失败，触发失败事件
				eval(obj.afterSaveEvent + "(0,'" + data.msg.text + "')");
			}
		}else{
			//失败，触发失败事件
			var error = "附件保存失败。";
	...
	this.setFileList = function(data){
		...
		$(data).each(
			function(index){
				...
				attach.id = data[index].id;
				attach.fileName = data[index].displayName + data[index].fileExt;
				attach.fileSize =  data[index].size;
				attach.fileURL = basePath +  "/" + data[index].absFilePath;
				attach.modTime =  data[index].uploadDate;
				//将附件添加到数组中，以文件名为关键字
	...
	this.init = function(refId){
		...
		if (this.permission.add == true){
			htmlCode += "<div class='fileinputs'>"
	           		+ "<div class='fileTextList'><input type='file'  class='file hidden' noscript='true' size='1' name='uploadFiles' onchange=\"WebAttachmentAddFile(this,'" + this.id + "');\"/></div>"
			...
			$.ajaxAttachmentUpload({
				...
				success: function(data, status){
					...
					if (afterSaveEventFunc != null && afterSaveEventFunc != ""){
						if (data.msg.code == 1){
							//成功，触发成功事件
							eval(afterSaveEventFunc + "(1,'" + data.msg.text + "')");
						}else{
							//失败，触发失败事件
							eval(afterSaveEventFunc + "(0,'" + data.msg.text + "')");
						}
					}
				},
